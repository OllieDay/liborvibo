...
static void
reverse_mac(unsigned char destination[static ETHER_ADDR_LEN], const unsigned char source[static ETHER_ADDR_LEN]);
...
orvibo_socket_discover(struct orvibo_socket *const socket) {
	...
	memcpy(&data[6], socket->mac_bytes, ETHER_ADDR_LEN);
...
parse_mac_string(const char *const mac) {
	unsigned char *buf = malloc(sizeof(*buf) * ETHER_ADDR_LEN);
	if (sscanf(mac,
			   "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
			   &buf[0], &buf[1], &buf[2], &buf[3], &buf[4], &buf[5]) == ETHER_ADDR_LEN) {
		return buf;
	}
	free(buf);
	return NULL;
